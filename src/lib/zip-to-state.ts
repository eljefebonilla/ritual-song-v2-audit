/**
 * Maps US zip codes to state abbreviations using 3-digit prefix ranges.
 * Covers all 50 states, DC, and US territories.
 *
 * Source: USPS Publication 65, 3-digit ZIP Code Prefix Matrix
 */
...
const ZIP_PREFIX_TO_STATE: Record<string, string> = {
  ...
  // VI - US Virgin Islands (shares 008 prefix with PR; 008xx is primarily PR)
  ...
  // NY - New York (AE/FPO military also uses 09x)
  ...
  // AS - American Samoa (shares 967 prefix with HI)
  // MP - Northern Mariana Islands (shares 969 prefix with GU)
};

/**
 * Resolves a US zip code (5-digit string) to a 2-letter state abbreviation.
 * Returns null for invalid or unrecognized zip codes.
 *
 * Handles military (APO/FPO) codes as AE/AA/AP.
 * Handles territories: PR, VI, GU.
 *
 * @example
 * zipToState("90210") // "CA"
 * zipToState("10001") // "NY"
...
/**
 * Returns all state abbreviations that have entries in the mapping.
 * Useful for validation and UI dropdowns.
 */
export function getAllMappedStates(): string[] {
  const states = new Set(Object.values(ZIP_PREFIX_TO_STATE));
