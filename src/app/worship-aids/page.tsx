...
interface SetlistRow {
  ...
  worship_aid_pdf_url: string | null;
  mass_event: {
    id: string;
    title: string;
    event_date: string;
...
export default function WorshipAidsPage() {
  ...
  const handleGenerate = useCallback(async (setlistId: string, massEventId: string) => {
    ...
    try {
      ...
      await fetch("/api/generate/worship-aid", {
        method: "POST",
  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto p-6">
        <h1 className="text-xl font-bold text-stone-900 mb-4">Worship Aids</h1>
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-stone-900">Worship Aids</h1>
          <p className="text-sm text-stone-500 mt-1">
            Assembly worship aids with cover pages, readings, and sheet music reprints.
          </p>
        </div>
      </div>
      ...
      {setlists.length === 0 ? (
        <div className="text-center py-12 text-stone-400">
          <p className="text-sm">No upcoming worship aids yet.</p>
          <p className="text-xs mt-1">
            Assign songs in the{" "}
            <Link href="/planner" className="text-stone-600 underline">
              Multi-Week planner
            </Link>{" "}
            and worship aids will auto-generate.
          </p>
        </div>
      ) : (
          ...
          {setlists.map((s) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 ml-3">
                {s.worship_aid_pdf_url ? (
                  <>
                    <a
                      href={s.worship_aid_pdf_url}
                      target="_blank"
